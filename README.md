# 🧠 JavaScript Fundamentals Projects 

This repository is created with a single goal in mind:  
**to strengthen my core JavaScript, HTML, and CSS fundamentals by building real-world components from scratch — without using any npm packages.**

As a **MERN Stack Developer**, I often used ready-made npm packages (like sliders, toast notifications, popup, etc.) without fully understanding how they work internally.  
This repo is my effort to **reverse that habit** and truly understand the **logic behind common UI components** by building them using **raw (vanilla) code**.

---

## 🎯 Purpose of This Repository

- Strengthen **core JavaScript concepts**
- Understand **how popular UI components work internally**
- Build confidence to create features **without relying on npm packages**
- Improve problem-solving and logical thinking
- Create a personal reference for future projects and interviews

---

## 🚫 What This Repository Does NOT Use

- ❌ No npm packages
- ❌ No frameworks
- ✅ Only **HTML, CSS, and Vanilla JavaScript**

---


---

## 🧩 Why This Matters

Understanding how things work **without abstractions** makes me a better developer.
After completing this repo, I will be able to:
- Rebuild common UI features anytime
- Customize logic freely
- Use libraries **by choice**, not dependency

---

